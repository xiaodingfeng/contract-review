/**
 * @file services/contractParser.js
 * @brief 合同条款树解析器，将合同文本解析为可独立审查的叶子条款列表
 *
 * 核心职责：
 * - 识别 章/编 → 条 → 款(X.X) 三级层级结构
 * - 将款折叠进所属条，以"条"作为默认审查单元
 * - 无条款结构时按段落 fallback 切分（目标 200-500 字/段）
 * - 提供中文数字转阿拉伯数字与 token 估算工具
 *
 * 关键实现：
 * - 正则匹配第X章/编、第X条、X.X 款三层标识
 * - 中文数字（含"十百千万"组合）转阿拉伯数字
 * - 长段落按中文句末标点切分后再合并至目标长度
 * - token 估算：中文 1 字≈1.5 token，英文 4 字符≈1 token
 *
 * 依赖关系：
 * - 上游：无外部依赖
 * - 下游：incrementalReview、审查服务等条款级处理逻辑
 */
