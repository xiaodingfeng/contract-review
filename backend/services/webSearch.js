...
const fetchSearchPage = async (url, options = {}) => {
    let currentUrl = url;
    let cookieJar = '';
    const maxHops = (options.maxRedirects ?? 5) + 1;
    ...
    for (let hop = 0; hop < maxHops; hop++) {
        const response = await axios.get(currentUrl, {
            ...
            params: hop === 0 ? options.params : undefined,
            maxRedirects: 0,
            validateStatus: () => true,
        });
        ...
        if (setCookie) {
            ...
            cookieJar.split('; ').forEach((kv) => {
                const k = kv.split('=')[0];
                if (k) merged.set(k, kv);
            });
            arr.forEach((c) => {
            ...
            cookieJar = [...merged.values()].filter(Boolean).join('; ');
        }
...
const parseBaiduMobileResults = (html) => {
    const results = [];
    const blockRe = /class="c-result\s+result"[^>]*data-log='([^']*)'/g;
    let match;
    while ((match = blockRe.exec(html)) !== null) {
        let mu = '';
        try {
            mu = JSON.parse(match[1]).mu || '';
        } catch {
            mu = match[1].match(/"mu":"([^"]+)"/)?.[1] || '';
        }
        if (!mu || !isHttpUrl(mu)) continue;
        if (mu.includes('recommend_list.baidu.com')) continue;
        mu = decodeHtml(mu);

        const startIdx = match.index;
        const nextIdx = html.indexOf('class="c-result result"', startIdx + 10);
        const block = html.slice(startIdx, nextIdx > 0 ? nextIdx : html.length);

        const title = block.match(/<!--s-text-->([\s\S]*?)<!--\/s-text-->/)?.[1]
            || block.match(/class="[^"]*\bcu-title\b[^"]*"[^>]*>([\s\S]*?)<\/p>/i)?.[1]
            || '';
        if (!title) continue;

        const snippet = block.match(/class="[^"]*\bsummary-text[^"]*"[^>]*>([\s\S]*?)<\/span>/i)?.[1]
            || block.match(/"generalLines":\[\{[^}]*"data":\[\{"text":"([^"]+)"/i)?.[1]
            || '';

        results.push(normalizeResult('baidu-html', { title, url: mu, snippet }));
    }
    return results;
};
...
const baiduSearch = async (query, count) => {
    const html = await fetchSearchPage('https://m.baidu.com/s', {
        params: {
            word: query,
            from: '1099a',
            sa: 'tb',
    ...
    return parseBaiduMobileResults(html).slice(0, count);
};
